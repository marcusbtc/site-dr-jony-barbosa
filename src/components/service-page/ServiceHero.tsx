...
const ServiceHero: React.FC<Props> = ({ service }) => {
  ...
  const intentLabel = service.intentLabel ?? 'Atendimento especializado';
  const heroIntro = service.heroIntro ?? service.description;
  const ctaLabel = service.ctaLabel ?? 'Agendar atendimento';
      ...
      <div className="page-container max-w-[1280px]">
        <div className={`page-hero ${heroImage ? 'grid items-center gap-10 text-left lg:grid-cols-2' : 'mx-auto max-w-4xl'}`}>
          <div className={heroImage ? 'order-2 lg:order-1' : ''}>
            <p className="eyebrow-label mb-4">{intentLabel}</p>
            ...
            {service.planCoverageText ? (
              <p className="mt-4 max-w-[40rem] font-['DM_Sans'] text-[14px] uppercase tracking-[0.1em] text-[var(--ds-color-brand-500)]">
                {service.planCoverageText}
              </p>
            ) : null}
