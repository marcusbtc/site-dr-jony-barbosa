...
const ServiceContent: React.FC<Props> = ({ service }) => {
  const introHeading = service.category === 'exame'
    ? `Como é o exame de ${service.title}?`
    : `Como funciona ${service.title}?`;
  const indicationHeading = service.category === 'exame'
    ? 'Quando este exame costuma ser solicitado?'
    : 'Quando esse atendimento é indicado?';
  const processHeading = service.category === 'exame'
    ? 'Como o exame é realizado?'
    : 'Como acontece o atendimento?';
  const benefitsHeading = service.category === 'exame'
    ...
    <section id="sobre" className="py-16 md:py-24">
      <div className="page-container max-w-4xl">
        <div className="page-section mb-10 p-8 md:p-10">
          <h2 className="mb-6 font-['EB_Garamond'] text-[40px] font-medium tracking-[0.4px] text-[var(--ds-color-text-strong)]">{introHeading}</h2>
        ...
        {service.whoNeedsIt && service.whoNeedsIt.length > 0 && (
          <div className="page-section mb-10 p-8 md:p-10">
            <h2 className="mb-6 font-['EB_Garamond'] text-[34px] font-medium tracking-[0.35px] text-[var(--ds-color-text-strong)]">{indicationHeading}</h2>
        ...
        {service.howItWorks && service.howItWorks.length > 0 && (
          <div className="page-section mb-10 p-8 md:p-10">
            <h2 className="mb-6 font-['EB_Garamond'] text-[34px] font-medium tracking-[0.35px] text-[var(--ds-color-text-strong)]">{processHeading}</h2>
        ...
        {service.benefits && service.benefits.length > 0 && (
          <div className="page-section p-8 md:p-10">
            <h2 className="mb-6 font-['EB_Garamond'] text-[34px] font-medium tracking-[0.35px] text-[var(--ds-color-text-strong)]">{benefitsHeading}</h2>
