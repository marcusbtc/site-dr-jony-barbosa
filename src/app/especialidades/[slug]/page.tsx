...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const service = servicesExtended.find(s => s.slug === params.slug && s.category === 'especialidade');
  if (!service) return {};
...
export async function generateStaticParams() {
  return servicesExtended
    .filter(s => s.category === 'especialidade')
    .map((s) => ({
      slug: s.slug,
    }));
}
...
export default function SpecialtyPage({ params }: Props) {
  ...
  return (
    <ServiceLayout service={service} category="especialidades" />
  );
}
